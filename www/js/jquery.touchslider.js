(function($, undefined) {
	window.touchSlider = function(options) {
			...
			return {
				...
				to: function(toIndex, opt) {
					...
					if (opt.pxInMs) {
						...
						duration = Math.min(Math.max(Math.round(Math.abs(crossLeft(scroller)) / opt.pxInMs), 300), duration);
						//duration = Math.max(Math.round(Math.abs(crossLeft(scroller)) / opt.pxInMs), 1000);
					}
					...
					toComplete = function() {
						...
						else{ if(options.onComplete) options.onComplete(true) }						
		...
		pagination.click(function() {
			step(pagination.index(this), options.onComplete);
		});

		// left/right button
		$(options.prev, container).click(function() {
			prev(options.onComplete);
		});
